...
function dioramaScene(kind) {
  return (ctx) => {
    ...
    if (kind === "learning") {
      ...
      hx.strokeStyle = "rgba(46,143,91,0.8)"; hx.lineWidth = 2;
      ...
      const holo = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ map: holoTex, transparent: true, opacity: 0.92, side: THREE.DoubleSide })
      );
      holo.position.set(0.95, 1.5, 0.4);
      holo.rotation.y = -0.7;
      stage.add(holo);
      // knowledge orbs orbiting the head
      const orbMat = new THREE.MeshStandardMaterial({ color: RB.accent, emissive: 0x1d7547, emissiveIntensity: 0.5 });
      const orbs = [];
      const linePos = new Float32Array(5 * 2 * 3);
      const lineGeo = new THREE.BufferGeometry();
      lineGeo.setAttribute("position", new THREE.BufferAttribute(linePos, 3));
      stage.add(new THREE.LineSegments(lineGeo, new THREE.LineBasicMaterial({ color: 0x2e8f5b, transparent: true, opacity: 0.4 })));
      for (let i = 0; i < 5; i++) {
        const o = new THREE.Mesh(new THREE.IcosahedronGeometry(0.055, 0), orbMat);
        stage.add(o); orbs.push(o);
      }
      update = (t) => {
        h.update(t);
        ...
        orbs.forEach((o, i) => {
          ...
          linePos[i * 6 + 0] = 0; linePos[i * 6 + 1] = 1.8; linePos[i * 6 + 2] = 0;
          linePos[i * 6 + 3] = o.position.x; linePos[i * 6 + 4] = o.position.y; linePos[i * 6 + 5] = o.position.z;
        });
