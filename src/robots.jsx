...
function dioramaScene(kind, zoom = 1) {
  return (ctx) => {
    ...
    if (kind === "embodied") {
      ...
      if (zoom < 1) { camera.position.set(10.4, 9, 11.6); camera.lookAt(0, 0.0, 0); }
    ...
    if (kind === "swarm") {
      if (zoom < 1) { camera.position.set(8, 5.7, 8.8); camera.lookAt(0, 0.4, 0); }
    ...
    if (kind === "gp") {
      ...
      if (zoom < 1) { camera.position.set(6.6, 5.6, 8.2); camera.lookAt(0, 0.5, 0); }
