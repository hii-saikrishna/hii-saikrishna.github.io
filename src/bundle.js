...
function dioramaScene(kind, zoom = 1) {
  return ctx => {
    ...
    if (kind === "embodied") {
      ...
      frame.position.set(-RW - 0.005, 0.85, 0.7);
      ...
      rBox(kitchen, rMat(0xe9e2d4, {
        roughness: 0.6
      }), 1.96, 0.06, 0.56, -1.3, 0.55, counterZ); // countertop
      ...
      [[-0.7, 0.1], [-0.7, -0.1], [-0.95, 0.1], [-0.95, -0.1]].forEach(([bx, bz]) => {
        // stove burners
      ...
      rBox(kitchen, rMat(0xdfe3e6, {
        roughness: 0.5,
        metalness: 0.3
      }), 0.52, 1.18, 0.5, -RW + 0.32, 0.59, counterZ); // fridge
