...
function buildMountainFooter({
  ...
  const matSnow = track(new THREE.MeshStandardMaterial({
    color: 0xf6fbf7,
    flatShading: true,
    roughness: 0.85
  }));
  ...
  const makeGlow = (c0, c1, scale, op) => {
    ...
    rg.addColorStop(1, "rgba(255,236,185,0)");
    g.fillStyle = rg;
  ...
  scene.add(glowOuter);
  scene.add(glowInner);
  const disc = new THREE.Mesh(track(new THREE.CircleGeometry(1.4, 40)), track(new THREE.MeshBasicMaterial({
    color: 0xffe7a0,
    fog: false,
    transparent: true
  })));
