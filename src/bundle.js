...
function buildWaterfallValley({
  ...
  const pos = geo.attributes.position;
  const col = new Float32Array(pos.count * 3);
  const cVerdant = new THREE.Color(0x6db85a),
    cForest = new THREE.Color(0x3d7a3e),
    cDeepGreen = new THREE.Color(0x2a5c30),
    cCliff = new THREE.Color(0x7a8a72),
    cWater = new THREE.Color(0xd0eaf5),
    cMist = new THREE.Color(0xc8e4d8);
  const outC = new THREE.Color(),
    baseC = new THREE.Color();
  for (let i = 0; i < pos.count; i++) {
    const x = pos.getX(i),
    ...
    const dd = 0.7,
      ...
      hz = heightAt(x, z + dd) - heightAt(x, z - dd);
    const slope = Math.sqrt(hx * hx + hz * hz) / (2 * dd);
    if (h < 4) baseC.copy(cVerdant).lerp(cForest, h / 4);else if (h < 8) baseC.copy(cForest).lerp(cDeepGreen, (h - 4) / 4);else if (h < 12) baseC.copy(cDeepGreen).lerp(cCliff, (h - 8) / 4);else baseC.copy(cCliff).lerp(cMist, Math.min(1, (h - 12) / 4));
    outC.copy(baseC);
  ...
  const mistTex = (() => {
    ...
    rg.addColorStop(0, "rgba(210,235,220,0.7)");
  ...
  return {
    update(t) {
      ...
      mists.forEach((m, i) => {
        m.material.opacity = 0.4 + 0.15 * Math.sin(t * 0.4 + i);
      });
    },
    dispose() {
      scene.background = null;
...
function buildMossyCliffs({
  ...
  const cMoss = new THREE.Color(0x5a8a4a),
    ...
    cTrickle = new THREE.Color(0xb8d8cc);
  const outC = new THREE.Color(),
    baseC = new THREE.Color();
  for (let i = 0; i < pos.count; i++) {
    const x = pos.getX(i),
